...
# 显示集:默认取「当日 净流入 Top12(右)/ 净流出 Top12(左)」自动模式(WHITELIST 为空)。
# WHITELIST 非空时改为只显示这些「概念关键词」(子串匹配实际板块名,取最短),其余并入「其他板块」。
# 当前需求是 Top-12 资金流,故 WHITELIST 置空;若想回到「只盯特定概念」模式,把 WHITELIST_PRESET
# 赋给 WHITELIST 即可。
WHITELIST: list[str] = []

# 备用的概念关键词预设(默认不启用;需要时 WHITELIST = WHITELIST_PRESET)。
WHITELIST_PRESET = [
...
HUB_LABEL     = "主力资金"
MAIN_LABEL    = "主力净额"        # 全市场主力资金净额节点(>0落左金 / <0落右灰)
OTHER_LABEL   = "其他板块"        # 配平左右总额的差额节点(颜色随所在侧:左绿/右红)
