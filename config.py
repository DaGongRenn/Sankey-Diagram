...
# 4. 桑基结构与稳定性参数
# ======================================================================
TOP_N = 15                  # 流入/流出各取 Top N(各 15 大净流入/净流出板块)

# 显示集白名单 = 用户「A股100大热门板块清单」(概念+行业)。WHITELIST 非空时,只显示语义
# 匹配到清单的板块(子串匹配实际板块名,取最短;东财/同花顺叫法不同的靠 WHITELIST_ALIAS 补别名),
# 其余(宽基/风格/资金属性等伪板块、未上清单的板块)一律不显示。置 [] 可回到「当日自动 Top-N」。
WHITELIST_PRESET = [
...
WHITELIST: list[str] = WHITELIST_PRESET
...
LAYOUT = {
    ...
    "hub_w":        70,     # hub 条带宽度
    "hub_pad":      16,     # 中枢光柱在节点堆叠范围外再上下各延伸的像素(盖住高列溢出/左右落差)
